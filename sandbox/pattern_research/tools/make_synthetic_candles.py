"""Generate a deterministic synthetic 1h OHLCV CSV (sandbox smoke-testing only).
...
real market information and must never be presented as a research result.
    ...
    python -m sandbox.pattern_research.tools.make_synthetic_candles \\
        --out /tmp/BTCUSDT_synth.csv --bars 17520 --seed 1
"""
...
def make_synthetic_candles(n_bars: int = 17520, seed: int = 1,
    ...
    index = pd.date_range(start=start, periods=n_bars, freq="1h", tz="UTC", name="timestamp")
...
def main(argv=None) -> int:
    ...
    df = make_synthetic_candles(args.bars, args.seed)
    os.makedirs(os.path.dirname(os.path.abspath(args.out)), exist_ok=True)
    df.to_csv(args.out, index_label="timestamp")
    print(f"Wrote {len(df)} synthetic candles to {args.out}")
    return 0
